"""
In-memory pipeline: GLiNER extract -> rank (BiEncoder + CrossEncoder) -> deduplicate.
Used by main.py to replace KeyBERT/GPT extraction. Models must be served on ports 6000, 6005, 6010.
"""
...
# Use the same preprocessing used by the older (faster) implementation.
# This reduces token/char count significantly before GLiNER sliding-window.
from process_entities import preprocess_content
...
from .nlp_serving_urls import BIENCODER_API_URL, GLINER_API_URL
from .bge_client import BGETritonClient

log = logging.getLogger(__name__)

# Parameters we log (GLiNER threshold is fixed in Gliner_.py at 0.12)
GLINER_THRESHOLD = 0.12
bge_client = BGETritonClient()


def extract_nlps_gliner(text: str, context_size: int = 600, step_size: int = 300) -> List[Dict]:
    """
    Extract entities from text using GLiNER (served on port 6000).
    ...
    log.info("[Step 1/3] GLiNER extraction — API=%s | threshold=%.2f | context_size=%d | step_size=%d | text_len=%d",
             GLINER_API_URL, GLINER_THRESHOLD, context_size, step_size, len(text))
    client = GLiNERClient()
...
def rank_entities_in_memory(
    ...
    """
    Rank entities by similarity to title using BiEncoder (6005) and CrossEncoder (6010).
    entities: list of {"text": str, "count": int}
    Returns list with "biencoder_score" and "crossencoder_score" (or None if CE unavailable).
    """
    ...
    log.info("[Step 2/3] Ranking — BiEncoder=%s | title=%r | entities=%d",
             biencoder_url, title[:50] + ("..." if len(title) > 50 else ""), len(entities))
...
def deduplicate_in_memory(
    ...
    """
    Deduplicate by semantic similarity using BiEncoder (6005).
    ...
    log.info("[Step 3/3] Deduplication — BiEncoder=%s | similarity_threshold=%.2f | input_entities=%d",
             biencoder_url, similarity_threshold, len(ranked_entities))
...
def run_pipeline(
    ...
    # Critical speed win (same as backup): preprocess before GLiNER to reduce chunk count.
    # This is usually a net quality win too (less noise).
    t_pre = time.perf_counter()
    preprocessed = preprocess_content(text)
    preprocess_seconds = time.perf_counter() - t_pre
    t_gliner = time.perf_counter()
    entities = extract_nlps_gliner(preprocessed, context_size=gliner_context_size, step_size=gliner_step_size)
