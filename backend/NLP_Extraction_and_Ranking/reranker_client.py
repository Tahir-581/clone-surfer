...
def _rerank_auth_headers() -> dict:
    key = (os.getenv("SURF_RERANK_API_KEY") or os.getenv("OPENAI_API_KEY") or "").strip()
    if not key:
        return {}
...
class RerankerClient:
    ...
    def _post_json(self, url: str, payload: dict, timeout: int) -> dict:
        try:
            headers = {"Content-Type": "application/json", **_rerank_auth_headers()}
            r = self._get_session().post(url, json=payload, headers=headers, timeout=timeout)
            r.raise_for_status()
            return r.json()
