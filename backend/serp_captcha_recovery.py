...
def next_proxy(current: Optional[str]) -> str:
    ...
    if not pool:
        return ""
    if not current or current not in pool:
        return pool[0]
...
def apply_recovery_rotation(
    ...
    *,
    ...
    _atomic_write_json(STATE_PATH, state)
    log.warning(
        "[CAPTCHA recovery] In-process retry #%d (hit %d) | proxy=%s | browser=%s | reason=%s",
        in_process_attempts,
        captcha_hits,
        next_proxy_url,
        next_browser,
...
def prepare_recovery(
    ...
    *,
    ...
    _atomic_write_json(STATE_PATH, state)
    log.warning(
        "[CAPTCHA recovery] Prepared hard-restart retry #%d | proxy=%s | browser=%s",
        captcha_hits,
        next_proxy_url,
        next_browser,
    )
